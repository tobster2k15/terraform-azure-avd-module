...
resource "azurerm_virtual_machine" "temp_vm_for_st_join"{
  ...
  network_interface_ids = ["${azurerm_network_interface.temp_nic.*.id[count.index]}"]
  admin_username        = var.local_admin
  admin_password        = var.local_pass
...
resource "azurerm_virtual_machine_extension" "domain_join_st" {
  count                      = var.fslogix_enabled == true ? 1 : 0
  name                       = "join-domain"
  virtual_machine_id         = azurerm_windows_virtual_machine.temp_vm_for_st_join.*.id[count.index]
  publisher                  = "Microsoft.Compute"
  type                       = "JsonADDomainExtension"
  type_handler_version       = "1.3"
  auto_upgrade_minor_version = true
...
resource "null_resource" "install_az_cli" {
  ...
  provisioner "local-exec" {
    command = <<EOF
    ./env/usr/bin/az resource invoke-action az vm delete \ --resource-group "${var.azurerm_resource_group.myrg_shd[count.index]} \ --name "${var.tetemp_vm_for_st_join[count.index]}" --force-deletion none
    EOF
  }
  depends_on = [
