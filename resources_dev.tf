...
### Resources
###>-<###>-<###>-<###>-<###>-<###>-<###>-<###>-<###>-<###>-<###>-<###>-<###
resource "azurerm_resource_group" "myrg_dev" {
  count    = var.dev_hostpool_enabled == true ? 1 : 0
...
resource "azurerm_virtual_desktop_host_pool" "pool_dev" {
  count                            = var.dev_hostpool_enabled == true ? 1 : 0
  ...
  name                             = local.vdpool_name_dev
  friendly_name                    = "Development Hostpool"
...
resource "azurerm_virtual_desktop_host_pool_registration_info" "token_dev" {
  count           = var.dev_hostpool_enabled == true ? 1 : 0
  hostpool_id     = azurerm_virtual_desktop_host_pool.pool_dev[count.index].id
...
resource "azurerm_virtual_desktop_workspace" "workspace_dev" {
  count               = var.dev_hostpool_enabled == true ? 1 : 0
  name                = local.workspace_dev
  location            = azurerm_resource_group.myrg_dev[count.index].location
...
resource "azurerm_virtual_desktop_application_group" "applicationgroup_dev" {
  count               = var.dev_hostpool_enabled == true ? 1 : 0
  name                = local.app_group_name_dev
  location            = azurerm_resource_group.myrg_dev[count.index].location
  resource_group_name = azurerm_resource_group.myrg[count.index].name
  type                = var.app_type != "RemoteApp" ? "Desktop" : "RemoteApp"
  host_pool_id        = azurerm_virtual_desktop_host_pool.pool_dev[count.index].id
  friendly_name       = "${var.usecase}"
...
resource "azurerm_virtual_desktop_workspace_application_group_association" "association_dev" {
  for_each             = toset(var.avd_access_dev)
  application_group_id = azurerm_virtual_desktop_application_group.applicationgroup_dev.id
  workspace_id         = azurerm_virtual_desktop_workspace.workspace_dev.id
}
# AAD group role and scope assignment.
# https://registry.terraform.io/providers/hashicorp/azurerm/latest/docs/resources/role_assignment
resource "azurerm_role_assignment" "rbac_dev" {
  for_each           = toset(var.avd_access_dev)
  scope              = azurerm_virtual_desktop_application_group.applicationgroup_dev.id
  role_definition_id = data.azurerm_role_definition.avduser_role.id
  principal_id       = data.azuread_group.avd_group_dev[each.value].id
}
...
resource "azurerm_virtual_desktop_application" "application_dev" {
  for_each                     = local.applications_dev
...
resource "azurerm_windows_virtual_machine" "vm_dev" {
  ...
  os_disk {
    name                 = "${local.osd_name_dev}${format("%03d", count.index + 1)}"
    caching              = "ReadWrite"
...
resource "azurerm_network_interface" "nic" {
  ...
  location            = azurerm_resource_group.myrg_dev[count.index].location
  ip_configuration {
    name                          = "${local.ipc_name_dev}-${format("%03d", count.index + 1)}"
